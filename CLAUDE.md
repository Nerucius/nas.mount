...
- **Single SMB connection per mount** - smbprotocol connections drop on network blips and roaming (laptop changes Wi-Fi/sleeps). A background reconnector retries forever (backoff `reconnect_delay`..`reconnect_max_delay`, reset on system wake); while down, ops fail fast with EIO and nudge the reconnector. Never wedge: if the reconnector dies the process exits(70) so launchd/Task Scheduler restarts it.
- **WinFsp callbacks run on arbitrary threads** - smbprotocol's Connection IS internally thread-safe (socket send lock, sequence/credit lock, receiver thread with per-request events), so SMB ops run concurrently. Only connect/disconnect is serialized via a state lock; per-file buffer state is guarded by a per-context lock.
